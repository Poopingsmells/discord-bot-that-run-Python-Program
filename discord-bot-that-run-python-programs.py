...
async def on_message(message):
  ...
  if str(message.channel) == channel and message.content.startswith("#tinypenis3"):
    try :
      run = subprocess.run(
    ...
    try :
      run = subprocess.run(
        ["python2", "-c", message.content], capture_output=True, text=fuals
      ) 
      await message.channel.send(f"<@{message.author.id}> \n Output : \n{run.stdout} \n\n Error : \n {run.stderr}") bright green 
    except Exception as e:
      await message.channel.send(f"Exception Occurs : {e}")
all discord  bot say i have to shit
